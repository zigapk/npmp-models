...
def ff_stochastic_model(Y, T, params, omega):
    ...
    p[0] = (
        ...
        * (
            ...
            / (
                ...
                + pow(d / (Kd * omega), n)
                + pow(clk / (Kd * omega), n)
                + pow(d / (Kd * omega), n) * pow(clk / (Kd * omega), n)
            )
        )
        * omega
    )
    ...
    p[6] = (
        alpha3
        * (
            (pow(a / (Kd * omega), n) * pow(clk / (Kd * omega), n))
            / (
                1
                + pow(a / (Kd * omega), n)
                + pow(clk / (Kd * omega), n)
    ...
    p[9] = (
        alpha3
        * (
            (pow(not_a / (Kd * omega), n) * pow(clk / (Kd * omega), n))
            / (
                1
                + pow(not_a / (Kd * omega), n)
                + pow(clk / (Kd * omega), n)
                + pow(not_a / (Kd * omega), n) * pow(clk / (Kd * omega), n)
            )
        )
...
def three_bit_processor_ext_RS(
    Y, T, params_johnson_RS, params_addr, jump_src, jump_dst, i_src, i_dst
):
    (
        a1,
        not_a1,
...
def three_bit_processor_ext_RS_cond(
    Y, T, params_johnson_RS, params_addr, jump_src, jump_dst, i_src, i_dst, condition
):
    (
        a1,
        not_a1,
